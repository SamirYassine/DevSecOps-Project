...
import { getAirZonesByOrganizationId } from "../../redux/actions/zone.action";
import axios from "axios";
import { apiBaseUrl } from "../../proxy";
import React from "react";
import { createAirForm } from "../../redux/actions/form.action";
...
const BuildAirForm = () => {
  ...
  const [selectedAirDevices, setSelectedAirDevices] = useState({});

  const handleDeviceNameChange = (event, index) => {
    const selectedId = event.target.value;
    const device = airDevices.find((d) => d.id === selectedId);
    setSelectedAirDevices((prev) => ({
      ...prev,
      [index]: device,
    }));
    setValue(`airSamplers.${index}.id`, device?.id ?? "");
    setValue(`airSamplers.${index}.name`, device?.deviceName ?? "");
    setValue(`airSamplers.${index}.calibration`, device?.calibration ?? "");
    setValue(`airSamplers.${index}.deviceType`, device?.deviceType ?? "");
  };
  useEffect(() => {
  ...
  useEffect(() => {
    if (initialized && fields.length === 0 && tableFields.length === 0) {
      append({});
      appendTable({});
    }
    setInitialized(true);
  }, [initialized, fields.length, tableFields.length]);

  const onSubmit = async (data) => {
    const formattedData = {
      ...data,
      samplingDate:
        (role !== "OrgAdmin" && role !== "") ? dayjs(selectedDate).format("YYYY-MM-DD") : "",
      expiryDate:
        (role !== "OrgAdmin" && role !== "") ? dayjs(selectedExDate).format("YYYY-MM-DD") : "",
      airSamplers: data.airSamplers.map((item) => ({
        id: item.id,
        name: item.name,
      })),
    ...
    try {
      const resultAction = await dispatch(createAirForm(formattedData));
      const message = resultAction.payload;
      console.log(message, "message");
      if (message === "Air created successfully") {
  ...
  return (
    <Box>
      <Header title="Air Sampling" subtitle="Prepare Form" />

      <div className="form-container">
        <div className="top-row">
          <img src={bcepharma} alt="" width="90px" className="first-image" />
          <TextField
            defaultValue={"SAMPLING OF VIABLE AIR PARTICLES"}
            multiline
            label="Form Name"
            {...register("formName", {
              required: (role !== "OrgAdmin" && role !== "") ? "Required" : false,
            })}
          />
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="config-section">
            <h3>Air Sampler Configuration</h3>
            <div className="input-air-config">
              {fields.map((field, index) => (
                <div className="input-air-container" key={field.id}>
                  <label>#{index + 1}</label>
                  ...
                  <TextField
                    ...
                    select
                    label="Device Name"
                    value={selectedAirDevices[index]?.id || ""}
                    onChange={(e) => handleDeviceNameChange(e, index)}
                  >
                    {airDevices.map((device) => (
                  ...
                  <label htmlFor="calibration">Calibration valide ? </label>
                  <TextField
                    disabled
                    value={
                      selectedAirDevices[index]
                        ? dayjs(
                            selectedAirDevices[index].validationDate
                          ).isAfter(dayjs())
                          ? "Yes"
                          : "No"
                        : ""
                  ...
                  />
                  <p className="error">{errors.calibration?.message}</p>
                  <label htmlFor="deviceType">Device Type </label>
                  <TextField
                    disabled
                    value={selectedAirDevices[index]?.deviceType || ""}
                  />
                  <p className="error">{errors.deviceType?.message}</p>
                  {index > 0 && (
